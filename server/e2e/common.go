...
func StartServer(t *testing.T, cfg *app.Config) *httpexpect.Expect {
	ctx := context.Background()
	repos, gateways := initTestReposAndGateways(t, ctx, cfg)
	return StartServerWithRepos(t, cfg, repos, gateways)
}

func initTestReposAndGateways(t *testing.T, ctx context.Context, conf *app.Config) (*repo.Container, *gateway.Container) {
	...
	if err := mongorepo.InitRepos(ctx, repos, db.Client(), "reearth_marketplace", mongox.IsTransactionAvailable(conf.DB)); err != nil {
		log.Fatalf("Failed to init mongo: %+v\n", err)
	}

	// File
	// fileRepo, err := gcs.NewFile(conf.GCS.Bucket, conf.GCS.AssetsBucket, conf.GCS.AssetsBaseURL)
	fileRepo, err := gcs.NewFile("marketplace.test.reearth.dev", "assets.marketplace.test.reearth.dev", "")
	if err != nil {
